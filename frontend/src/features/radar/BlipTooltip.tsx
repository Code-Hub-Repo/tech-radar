// Hover/focus preview: entry name + ring · quadrant, positioned by the pure clampTooltip
// edge-clamp. Rendered as a <foreignObject> sibling to Blip's own button -- the same SVG->HTML
// re-entry technique 02-04 established for the button -- so its position stays pixel-consistent
// with the radar's viewBox scaling with zero separate screen-pixel math. isNew/movement text
// is a clean seam left for 02-07.
...
// Fixed footprint estimate in the radar's own SVG user-units -- generous enough for the
// longest entry name + "Languages & Frameworks" quadrant label without measuring the DOM.
const TIP_SIZE = { w: 200, h: 56 }
